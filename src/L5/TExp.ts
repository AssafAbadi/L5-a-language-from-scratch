...
*/
import { chain, concat, filter, ifElse, is, map, uniq } from "ramda";
...
import { makeBoolExp } from "./L5-ast";
...
export const makeUnionTExp = (params: TExp[]): UnionTExp => ({tag: "UnionTExp", components: params});
...
const parseCompoundTExp = (texps: Sexp[]): Result<TExp> => {
    ...
    return (pos === -1)  ? (texps.length === 0) ? makeFailure(`Empty type expression`) :
            (texps[0]==="union") ? parseUnionTExp(texps.slice(1)) :
...
export const comparTags=(a:Sexp, b:Sexp): number =>{
...
//   bind(mapResult(parseTExp, texps.sort(comparTags)), (unionTEs: TExp[]) => makeOk(makeUnionTExp(unionTEs)))
export const parseUnionTExp = (texps: Sexp[]): Result<TExp> => {
   if(texps.length === 0) 
   {
   return makeFailure(`Empty union type expression`)
   }  else if(texps.length === 1)
    {
       return makeFailure(`Union type expression with only one type`)
    } else if(texps.length === 2)
    {
        if(texps[0].toString()===texps[1].toString())
        {
            if(texps[0]==="boolean")
            return makeOk(makeBoolTExp());
            else if(texps[0]==="number")
            return makeOk(makeNumTExp());
            else if(texps[0]==="string")
            return makeOk(makeStrTExp());
        }

        const union = mapResult(parseTExp, texps.sort(comparTags));
        if(isOk(union))
        {
            const unionTEargs = map((x) =>isUnionTExp(x) ? x.components: x, union.value).flat();
           const args= unionTEargs.filter((item,
                index) => unionTEargs.indexOf(item) === index);
            return makeOk(makeUnionTExp(args));
        }
    } 
       
        return makeFailure(`Union type expression with more than two types`);
...
export const unparseUnion = (components: TExp[]): Result<string> => {
    
        if(components.length === 2)
        {
            return bind(unparseTExp(components[0]),(first:string)=> bind(unparseTExp(components[1]),(second:string)=>{if(first!=second) return makeOk(`(union ${first} ${second})`);else{if(first==="boolean") return makeOk("boolean");else if(first==="number") return makeOk("number");else return makeOk("string");}}));
        }
        else
        {
            return bind(unparseTExp(components[0]),(first:string)=> bind(unparseUnion(components.slice(1)),(second:string)=>makeOk(`(union ${first} ${second})`)));
        }
    }
